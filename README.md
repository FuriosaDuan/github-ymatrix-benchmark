...
## 22 条查询-具体业务
查询	业务问题
Q01	截止某个发货日期，按退货标记和订单明细状态汇总数量、价格、折扣、税费和订单行数。主要测试大规模明细聚合。
Q02	在欧洲地区，找出指定尺寸和类型商品中，供应成本最低的供应商及其信息。主要测试多表连接、区域过滤和最小值子查询。
Q03	对 BUILDING 市场客户，在指定日期前下单、但在指定日期后发货的订单，计算收入并取前 10 名。
Q04	统计指定季度内，存在“承诺日期早于实际收货日期”的订单，并按订单优先级分组。
Q05	统计亚洲地区一年内各国家的销售收入，要求供应商和客户来自同一国家。
Q06	统计指定一年内、折扣在 5% 到 7%、数量小于 24 的订单明细收入。主要测试单表过滤和聚合。
Q07	统计法国和德国之间相互供货产生的收入，按供应国、客户国和年份分组。
Q08	统计美洲地区客户购买指定商品类型时，巴西供应商的市场份额。
Q09	对名称包含 green 的商品，按供应商所在国家和订单年份计算利润。
Q10	统计指定季度内发生退货的客户收入，返回收入最高的前 20 个客户。
Q11	统计德国供应商所供应商品的库存价值，只保留超过德国整体库存价值阈值的商品。
Q12	统计 MAIL 和 SHIP 两种运输方式下，高优先级和低优先级订单明细数量。
Q13	统计“每个客户拥有多少订单”的客户分布。也就是订单数为 0、1、2……的客户各有多少。
Q14	统计指定月份内促销商品收入占总收入的百分比。
Q15	统计指定季度内收入最高的供应商。
Q16	按商品品牌、类型和尺寸统计供应商数量，并排除投诉供应商和指定商品条件。
Q17	统计指定品牌、指定包装商品中，数量低于该商品平均数量 20% 的销售收入。
Q18	找出明细数量总和超过 200 的大订单，并返回客户、订单和订单明细数量，取前 100 个。
Q19	根据三组品牌、包装、数量和尺寸条件，统计空运方式、上门交付商品的收入。
Q20	找出加拿大供应商中，有能力供应名称以 Part 1 开头商品的供应商。库存可用量需要超过历史发货量的一半。
Q21	找出沙特阿拉伯供应商中，存在“等待交付”情况的供应商，并统计等待次数。
Q22	按电话国家码统计没有订单、账户余额高于平均值的客户数量和账户余额总额。
